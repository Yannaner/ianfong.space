...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Initialize animations when Resume section becomes active
    ...
    const observer = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        if (mutation.attributeName === 'class' && 
            resumePage.classList.contains('active')) {
          animateSkillBars();
        }
      });
    });
    
    observer.observe(resumePage, { attributes: true });
  ...
  // Add active state to navbar items when scrolling
  ...
  function addRippleEffect() {
    const buttons = document.querySelectorAll('button, .navbar-link, .social-link');
